...
const fetchJson = async <T>(url: string): Promise<T> => {
  ...
  try {
    ...
    if ((error as Error).name === 'AbortError') {
      throw new Error('The request to the flight provider timed out. Please try again.');
    }
    throw error;
  } finally {
...
const resolveAirport = async (query: string, apiKey: string, baseUrl: string): Promise<ResolvedAirport> => {
  ...
  let data: AviationStackResponse<AirportLookupResult>;
  try {
    data = await fetchJson<AviationStackResponse<AirportLookupResult>>(
      `${baseUrl}/airports?${params.toString()}`
    );
  } catch (error) {
    if (error instanceof Error) {
      throw new Error(`Unable to resolve airport "${trimmedQuery}": ${error.message}`);
    }
    throw new Error(`Unable to resolve airport "${trimmedQuery}" due to an unknown error.`);
  }
...
const estimatePrice = (distanceKm: number): number => {
  const baseFare = 60;
  const perKm = 0.18;
  const variabilitySeed = Math.abs(Math.sin(distanceKm / 100));
  const variability = 0.85 + variabilitySeed * 0.3; // between 0.85x and 1.15x
...
const fetchFlights = async (filters: Filters): Promise<Flight[]> => {
  ...
  try {
    ...
    const searchParams = new URLSearchParams({
      ...
      limit: DEFAULT_FLIGHT_LIMIT.toString(),
